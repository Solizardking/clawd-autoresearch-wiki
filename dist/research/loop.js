...
export class ResearchLoop {
    ...
    // ── Main Loop ─────────────────────────────────────────────────────────
    async run(maxExperiments = 100, experimentBudgetMs = 300_000) {
        this.isRunning = true;
        ...
        while (this.isRunning && experimentCount < maxExperiments) {
            experimentCount++;
            const experimentPad = new Scratchpad(`Experiment #${experimentCount}`, "./.clawvault/scratchpad");
            ...
            try {
                ...
                // 2. Mutate strategy
                const newParams = await this.mutateStrategy(this.currentStrategy, hypothesis);
                // 3. Backtest
                ...
                if (accepted) {
                    ...
                    console.log(`   ✅ ACCEPTED — new best metric: ${result.metric.toFixed(4)}`);
                    experimentPad.addDecision("ACCEPTED");
                    await this.saveStrategy(newParams);
                }
                else {
                    console.log(`   ❌ REJECTED — metric ${result.metric.toFixed(4)} < best ${this.bestMetric.toFixed(4)}`);
                ...
                console.error(`   ⚠️  Experiment failed: ${e.message}`);
            }
    ...
    async generateHypothesis(program, experimentNumber) {
        const lessons = await this.vault.recall("strategy improvement lesson", { category: "lessons", limit: 5 });
        const lessonContext = lessons.map((l) => `- ${l.title}`).join("\n");
        const expHistory = this.experiments
            .slice(-5)
            .map((e) => `- ${e.hypothesis}: ${e.accepted ? "✅" : "❌"} (metric: ${e.result?.metric.toFixed(4) ?? "N/A"})`)
            .join("\n");
        const prompt = `
...
`;
    ...
    // ── Strategy Mutation ─────────────────────────────────────────────────
    async mutateStrategy(current, hypothesis) {
        const prompt = `
...
`;
    ...
    backtestStrategy(candles, params) {
        const trades = [];
    ...
    computeResult(trades) {
        ...
        const avgPnlPct = trades.reduce((s, t) => s + t.pnl, 0) / trades.length;
        let peak = 1;
        let equity = 1;
        let maxDrawdown = 0;
        ...
        const comparison = delta > 0.01 ? "better" : delta < -0.01 ? "worse" : "neutral";
        return {
...
`.trim();
        ...
        if (exp.accepted && exp.result) {
            await this.vault.remember(`Strategy improvement: ${exp.hypothesis}. ` +
                `Win rate: ${(exp.result.winRate * 100).toFixed(1)}%, Sharpe: ${exp.result.sharpe.toFixed(3)}.`, { category: "lessons", score: 0.85 });
        }
    }
...
`.trim();
    ...
    async loadStrategy() {
        try {
            ...
            if (jsonMatch) {
                const loaded = JSON.parse(jsonMatch[1]);
