...
export class StrategyRegistry extends EventEmitter {
     ...
     */
    async updateParams(newParams, opts) {
        const previous_params = { ...this.state.active_params };
        const merged = { ...previous_params, ...newParams };
        // Compute delta
        const delta = {};
        for (const key of Object.keys(newParams)) {
            if (newParams[key] !== previous_params[key]) {
                delta[key] = newParams[key];
            }
        }
     ...
     */
    async recordMetric(value, metricName) {
        if (metricName)
            this.state.metric_name = metricName;
     ...
     */
    async resetToDefaults(reason = 'Manual reset') {
    ...
    async autoOptimize(opts) {
        ...
        await this.updateParams({ [pick.key]: pick.value }, { reason: pick.reason, triggered_by: 'auto_optimize', current_metric: metric });
    }
    ...
    async loadFromSupabase() {
        ...
        try {
            ...
            logger.warn('[StrategyRegistry] Supabase load failed, using defaults', { err: err.message });
        }
    }
    async saveToSupabase() {
        ...
        try {
            const { error } = await this.supabase
                .from(TABLE)
                .upsert({ strategy_key: STRATEGY_KEY, state: this.state, updated_at: new Date().toISOString() }, { onConflict: 'strategy_key' });
            if (error) {
                logger.error('[StrategyRegistry] Supabase save failed', { error: error.message });
            }
        }
        catch (err) {
            logger.warn('[StrategyRegistry] Supabase save exception', { err: err.message });
        }
    }
    // ── File I/O (fallback) ────────────────────────────────────────────────────
    async loadFromFile() {
    ...
    async saveToFile() {
        ...
        try {
            ...
            logger.error('[StrategyRegistry] File save failed', { err: err.message });
        }
    }
}
// Singleton
