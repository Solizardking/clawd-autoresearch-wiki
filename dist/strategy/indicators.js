 ...
 */
export function latestRSI(bars, period = 14) {
 ...
 * Latest EMA value.
 */
export function latestEMA(bars, period) {
    const closes = bars.map(b => b.close);
 ...
 */
export function emaCrossState(bars, fastPeriod, slowPeriod) {
    const closes = bars.map(b => b.close);
    const fastArr = calculateEMA(closes, fastPeriod);
    const slowArr = calculateEMA(closes, slowPeriod);
    const len = bars.length;
    const ema_fast = fastArr[len - 1] ?? null;
    const ema_slow = slowArr[len - 1] ?? null;
    const prevFast = fastArr[len - 2] ?? null;
    const prevSlow = slowArr[len - 2] ?? null;
 ...
 */
export function avgVolume(bars, n = 20) {
