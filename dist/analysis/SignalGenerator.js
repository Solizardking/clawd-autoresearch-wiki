...
class SignalGenerator {
    ...
    async cryptoSignal(token) {
        ...
        try {
            ...
            logger.warn('[SignalGenerator] Birdeye overview failed', { err: err.message });
        }
        ...
        try {
            ...
            logger.warn('[SignalGenerator] Birdeye OHLCV failed', { err: err.message });
        }
        ...
        try {
            ...
            logger.debug('[SignalGenerator] Aster funding failed', { err: err.message });
        }
        ...
        if (bars.length >= 30) {
            try {
                ...
                if (!stratSignal.filtered) {
                    ...
                    for (const r of stratSignal.reasons)
                        thesisParts.push(`  • ${r}`);
                }
                else {
                ...
                logger.warn('[SignalGenerator] Strategy evaluation failed', { err: err.message });
            }
        }
    ...
    async equitySignal(ticker) {
        ...
        try {
            ...
            logger.warn('[SignalGenerator] Equity snapshot failed', { ticker, err: err.message });
        }
        // 2. Fundamentals
        try {
