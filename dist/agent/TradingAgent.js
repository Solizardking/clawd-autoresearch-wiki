...
import { signalGenerator } from '../analysis/SignalGenerator.js';
...
export class TradingAgent extends EventEmitter {
    ...
    async start() {
        ...
        strategyRegistry.on('params:updated', ({ entry }) => {
        ...
        strategyRegistry.on('metric:improved', ({ value, metric_name }) => {
        ...
        this.cycleTimer = setInterval(() => {
            if (this.running)
                this.runCycle().catch(err => logger.error('Cycle error', { err: err.message }));
        }, this.cfg.cyclePeriodMs);
        // Schedule learning
        this.learnTimer = setInterval(() => {
            if (this.running)
                this.runLearning().catch(err => logger.error('Learning error', { err: err.message }));
    ...
    async runCycle() {
        ...
        for (const address of this.cfg.cryptoWatchlist) {
            await this.analyzeAndTrade(address, 'crypto').catch(err => logger.error('Crypto analysis failed', { address, err: err.message }));
        }
        // 5. Scan equity watchlist
        for (const ticker of this.cfg.equityWatchlist) {
            await this.analyzeAndTrade(ticker, 'equity').catch(err => logger.error('Equity analysis failed', { ticker, err: err.message }));
        }
    ...
    async analyzeAndTrade(asset, assetClass) {
        ...
        if (this.recentSignals.length > 200)
    ...
    async gatherMacroContext() {
        try {
            ...
            await memoryEngine.remember({
                ...
                raw_data: globalCrypto,
                metadata: { snapshot_type: 'sentiment' },
            });
            logger.debug('Macro context updated', globalCrypto);
        }
        catch (err) {
            logger.warn('Failed to gather macro context', { err: err.message });
        }
    }
    // ── Epistemological reporting ──────────────────────────────
    logEpistemicState(asset, state) {
    ...
    async getTradeHistory(limit = 50) {
        try {
            ...
            logger.warn('[TradingAgent] getTradeHistory failed', { err: err.message });
            return [];
        }
    }
    async runCycleNow() {
     ...
     */
    async getKnowledgeSummary(asset) {
        if (asset) {
        ...
        return [
            ...
            'Assets:',
            ...assets.slice(0, 10).map(a => `  • ${a.entity_id} (${a.entity_type}): price=${a.has_price_data}, fundamentals=${a.has_fundamentals}, patterns=[${(a.learned_patterns ?? []).join(',')}]`),
        ].join('\n');
    }
}
