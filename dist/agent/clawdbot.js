 ...
 */
import { callLlmMultiTurn } from "../model/llm.js";
...
import { Scratchpad, TokenCounter } from "./scratchpad.js";
...
export class ClawdBot {
    ...
    constructor(config) {
        ...
        this.helius = new HeliusConnector(config.heliusApiKey, config.heliusRpcUrl, config.heliusWsUrl);
        ...
        this.scratchpad = new Scratchpad("ClawdBot OODA session", "./.clawvault/scratchpad");
    }
    // ── Boot ──────────────────────────────────────────────────────────────
    async wake() {
        ...
        if (checkpoint) {
            ...
            await this.remember(`Woke up from checkpoint. Last observation: ${checkpoint.lastObservation}`, { category: "decisions" });
        }
        try {
    ...
    // ── OODA Loop ─────────────────────────────────────────────────────────
    async startOODALoop() {
        console.log(`🔄 OODA loop started (${this.oodaIntervalMs / 1000}s interval, GPT-5.4 reasoning)`);
        const loop = async () => {
            if (!this.isRunning)
                return;
            try {
...
`;
        ...
        // Store
        await this.vault.remember(`## OODA Observation\n\n${observationStr}\n\n### Decision\n${JSON.stringify(decision, null, 2)}`, { category: "decisions", score: decision.confidence });
        if (Math.random() < 0.2)
    ...
    async observe() {
        ...
        const watchlistData = await Promise.allSettled(this.watchlist.map(async (mint) => {
    ...
    formatObservation(obs) {
        ...
        if (obs.watchlist.length > 0) {
            ...
            for (const token of obs.watchlist) {
                ...
                lines.push(`**${token.symbol}** $${token.price.toFixed(6)} | 24h: ${token.priceChange24h.toFixed(2)}% | ` +
        ...
        if (obs.perpsDigest) {
            ...
            if (d.topByVolume.length > 0) {
                ...
                for (const t of d.topByVolume.slice(0, 5)) {
                    lines.push(`  ${t.symbol}: $${t.lastPrice} | 24h: ${t.priceChangePercent}% | Vol: $${parseFloat(t.quoteVolume).toLocaleString()}`);
                }
            }
        }
        return lines.join("\n");
    ...
    async recordTrade(trade) {
        ...
        if (trade.outcome) {
            const lesson = await this.chat(`Trade completed: ${trade.outcome === "win" ? "WIN" : "LOSS"} on ${trade.token}. ` +
...
`;
        ...
        await this.vault.remember(`# Research: ${overview.symbol}\n\n${analysis}`, {
            category: "research",
