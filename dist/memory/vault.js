...
export class ClawVault {
    ...
    async init() {
        ...
        try {
            ...
            this.graphIndex = JSON.parse(raw);
        }
        catch {
    ...
    // ── Graph Traversal ──────────────────────────────────────────────────
    async linkEntries(fromId, toId, weight = 1.0) {
    ...
    async traverseGraph(startId, depth = 2) {
        ...
        const traverse = async (id, currentDepth) => {
            if (visited.has(id) || currentDepth < 0)
                return;
            visited.add(id);
...
`.trim();
    ...
    async getTradeHistory(token, limit = 20) {
    ...
    scoreContent(content) {
        ...
        if (lower.includes("pnl") || lower.includes("profit") || lower.includes("loss"))
            score += 0.15;
        if (lower.includes("pattern") || lower.includes("insight"))
            score += 0.15;
        if (lower.includes("learned") || lower.includes("mistake"))
            score += 0.2;
        if (content.includes("```"))
            score += 0.1; // code = structured
        if (content.length > 500)
            score += 0.1;
        return Math.min(score, 1.0);
    }
    computeRelevance(query, entry) {
        ...
        for (const word of queryWords) {
            ...
            if (titleWords.has(word))
                matches += 2; // title match weighted more
    ...
    async buildContextProfile(query) {
