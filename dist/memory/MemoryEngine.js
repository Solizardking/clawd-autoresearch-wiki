...
class MemoryEngine {
    ...
    async recall(query, opts) {
        ...
        if (this.supabase) {
            try {
                let q = this.supabase
                    ...
                    .limit(limit);
                if (opts?.asset)
                    q = q.eq('asset', opts.asset);
                if (opts?.memory_type)
                    q = q.eq('memory_type', opts.memory_type);
                if (query)
        ...
        return this.memoryStore
            .filter(m => {
            if (opts?.asset && m.asset !== opts.asset)
                return false;
            if (opts?.memory_type && m.memory_type !== opts.memory_type)
                return false;
            if (query && !m.content.toLowerCase().includes(lower) && !m.topic.toLowerCase().includes(lower))
                return false;
            return true;
        })
    ...
    async whatDoIKnow(asset) {
        ...
        if (!hasPriceData)
            gaps.push('No recent price data');
        if (!hasOnChain)
            gaps.push('No on-chain activity data');
        if (!hasFundamentals)
            gaps.push('No fundamental analysis');
        if (!hasPatterns)
            gaps.push('No learned patterns yet');
        // Confidence summary
    ...
    async getTradeHistory(asset, limit = 20) {
        await this.init();
        if (this.supabase) {
            try {
                let q = this.supabase
