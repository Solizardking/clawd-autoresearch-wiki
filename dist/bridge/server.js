...
// ── Python Integration ───────────────────────────────────────────────
async function triggerPythonTraining(description) {
    return new Promise((resolve) => {
        ...
        proc.on("close", async (code) => {
            ...
            if (agent) {
                await agent.remember(`## Python Training Result\n\nval_bpb: ${metric}\n${description ?? ""}\n\n${stdout.slice(-500)}`, { category: "research", score: 0.8 });
            }
        });
...
async function handleRequest(req, res) {
    ...
    try {
        ...
        if (pathname === "/api/python/result" && method === "POST") {
            ...
            pythonResults.push(result);
            if (agent) {
                await agent.remember(`## Python Result\n\nMetric: ${result.metric}\n${result.description}`, { category: "research", score: 0.7 });
            }
        ...
        if (pathname === "/api/dexter/research" && method === "POST") {
            ...
            try {
                ...
                error(res, `Dexter research failed: ${e.message}`, 500);
            }
            return;
        }
        // ── Dexter Status ──────────────────────────────────────────
